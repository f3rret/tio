...
const Landing = ({pointerdown, fill, children, x, y, width}) => {
    ...
    return <Container zIndex={4} x={x} y={y + sc} alpha={.9} interactive={true} pointerdown={(e) => pointerdown(e)} cursor='pointer' mouseover={(e) => e.target.alpha = 1} mouseout={(e) => e.target.alpha = .9}>
        {children}
        <Text text={'⯆'} y={0} x={0}
