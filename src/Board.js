...
export function TIOBoard({ ctx, G, moves, events, undo, playerID, sendChatMessage, chatMessages }) {
  ...
  const StrategyCard = ({card, idx, style}) => {
    ...
    return <Card onClick={()=>setStratUnfold((stratUnfold & i) === i ? stratUnfold - i:stratUnfold + i)} 
              style={{...style, opacity: card.exhausted ? '.5':'.95', border: 'none', background: 'none', position: 'relative', 
  ...
  const TileContent = ({element, index}) => {
    ...
    return <Container x={firstCorner.x + stagew/2 + 7.5 - element.w/2 - element.w/4} y={firstCorner.y + stageh/2 + 7.5}>
        ...
        {element.tdata.tokens !== undefined && element.tdata.tokens.length > 0 && element.tdata.tokens.map( (t, i) =>{
